...
<table>
 ...
 <tr>
  ...
  <tr>
    <td colspan="2"> 
    <font style="font-weight: bold; font-size: medium">
    <?php 
    if(isset($_SESSION['applicant']))
    {
		...
		$are_applied=$applicantModel->get_applied_job_by_job_id($applicant_id, $job_id);
    	if($are_applied)
    	{
	    	if($lang=='ar')
	    	{
	    		echo "انت متقدم لهذة الوظيفة";
	    	}
	    	else
	    	{
	    		echo "you are applied to this job.";
	    	}
    	}
		else
    	{
	    ?>
	    <input type="submit" name="smt_apply" id="smt_apply" value="<?php echo $smt_apply_lang; ?>" onclick="return displaymessage('<?php echo $lang; ?>')"/> </td>
	    <?php
    	} 
    }
    ?>
