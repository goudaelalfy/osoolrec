...
if(isset( $this->data['company_jobs']))
{
	...
	if(count($company_jobs)>0)
	{
	 	...
	 	foreach($company_jobs as $company_job)
	    {
			...
			<td>$date_from </td>
			<td>$date_to </td>
			<td><a href='".site_url."/job/admin_edit/$id'>$edit_link_lang</a> </td>
			<td><a href='".site_url."/job/admin_delete/$id' onclick='return confirm(\"Are You Want To Delete This Row?\");'>$delete_link_lang</a>  </td>
			</tr>
			";
	    }
